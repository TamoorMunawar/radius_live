...
class _AnnouncementScreenState extends State<AnnouncementScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocConsumer<AnnouncementCubit, AnnouncementState>(builder: (context, state) {
        print("announcement state $state ");
        if (state is AnnouncementSuccess) {
          return (state.announcement?.isEmpty ?? false)
              ? Center(
                  child: Container(
                    ...
                    child: Text(
                      "No Data Found".tr(),
                      style:
                          TextStyle(color: GlobalColors.textFieldHintColor, fontSize: SizeConfig.width(context, 0.06)),
                    ),
                  ),
                )
              : ListView.separated(
                  ...
                  itemCount: (isLoadingMore) ? state.announcement?.length ?? 0 + 1 : state.announcement?.length ?? 0,
                );
        }
